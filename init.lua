...
require('lazy').setup({
    ...
    { -- Collection of various small independent plugins/modules
        ...
        config = function()
            ...
            local cursor_in_list = function()
                local list_nodes = { markdown = { 'list', 'list_item', 'task_list_item' } }
                local valid = list_nodes[vim.bo.filetype]
                if not valid then
                    return false
                end
                local node = vim.treesitter.get_node() -- node under cursor
                while node do
                    local t = node:type()
            ...
            local cursor_at_start = function()
                return vim.api.nvim_win_get_cursor(0)[2] == 0
            end

            map_multistep('i', '<Tab>', {
                {
                    condition = cursor_at_start,
                    action = function()
                        return '<Tab>'
                    end,
                },
                {
                    condition = cursor_in_list,
                    action = function()
